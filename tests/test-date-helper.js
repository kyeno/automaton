...
console.log('── day-period boundaries ──\n')
{
    ...
    assert(temporal.getCurrentTimePeriod(jan(4, 59)) === 'night', 'Jan 04:59 still night')
    assert(temporal.getCurrentTimePeriod(jan(5, 0)) === 'morning', 'Jan 05:00 starts morning -- pre-dawn included')
    assert(temporal.getCurrentTimePeriod(jan(10, 59)) === 'morning', 'Jan 10:59 still morning')
    assert(temporal.getCurrentTimePeriod(jan(11, 0)) === 'noon', 'Jan 11:00 starts noon')
    assert(temporal.getCurrentTimePeriod(jan(12, 59)) === 'noon', 'Jan 12:59 still noon')
    assert(temporal.getCurrentTimePeriod(jan(13, 0)) === 'afternoon', 'Jan 13:00 starts afternoon')
    assert(temporal.getCurrentTimePeriod(jan(17, 59)) === 'afternoon', 'Jan 17:59 still afternoon -- evening never before 6pm')
    assert(temporal.getCurrentTimePeriod(jan(18, 0)) === 'evening', 'Jan 18:00 starts evening')
